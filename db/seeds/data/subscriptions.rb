...
users.each_with_index do |user, index|
  ...
  when 0..4
    ...
    Billing::Subscription.find_or_create_by!(user: user, plan_version: free_plan_version) do |sub|
      sub.start_date = Faker::Date.between(from: 2.years.ago, to: 1.year.ago)
  ...
  when 5..14
    ...
    Billing::Subscription.find_or_create_by!(user: user, plan_version: basic_plan_version_v2) do |sub|
      sub.start_date = Faker::Date.between(from: 1.year.ago, to: Date.today)
      sub.status = Billing::Subscription::Active
    ...
    if rand < 0.7 # 70% chance to have a past subscription
      Billing::Subscription.find_or_create_by!(user: user, plan_version: basic_plan_version_v1) do |sub|
        sub.start_date = Faker::Date.between(from: 3.years.ago, to: 2.years.ago)
  ...
  when 15..users.size
    ...
    Billing::Subscription.find_or_create_by!(user: user, plan_version: premium_plan_version_v1) do |sub|
      sub.start_date = Faker::Date.between(from: 6.months.ago, to: Date.today)
      sub.status = Billing::Subscription::Active
